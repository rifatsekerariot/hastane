...
import { Monitor, Plus, Check, Link as LinkIcon } from 'lucide-react';
...
const AdminPanel = () => {
    ...
    const fetchData = async () => {
        try {
            const [roomsRes, devicesRes] = await Promise.all([
                axios.get('http://localhost:3005/api/admin/rooms'),
                axios.get('http://localhost:3005/api/admin/devices')
            ]);
    ...
    const handleCreateRoom = async (e) => {
        e.preventDefault();
        try {
            await axios.post('http://localhost:3005/api/admin/rooms', newRoom);
    ...
    const handleCreateDevice = async (roomNumber) => {
        ...
        if (!name) return;
        try {
            await axios.post('http://localhost:3005/api/admin/devices', {
                deviceName: name,
                roomNumber
            });
            fetchData();
